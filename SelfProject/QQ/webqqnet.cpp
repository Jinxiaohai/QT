...
#include <QtNetwork>
#include "webqq.h"
WebQQNet::WebQQNet(QObject* parent) : QObject(parent) {
  ...
  connect(httpPoll, SIGNAL(finished(QNetworkReply*)), this,
          SLOT(httpPollFinished(QNetworkReply*)));
  connect(httpExtInfo, SIGNAL(finished(QNetworkReply*)), this,
          SLOT(httpExtInfoFinished(QNetworkReply*)));
  QTime time;
...
void WebQQNet::pollMsg() {
  ...
  QByteArray postData;
  postData.append("r=" + QUrl::toPercentEncoding(
                             QString("{\"clientid\":\"%1\",\"psessionid\":\"%"
                                     "2\",\"key\":0,\"ids\":[]}")
                                 .arg(clientid)
                                 .arg(psessionid)));
  postData.append(
      QString("&clientid=%1&psessionid=%2").arg(clientid).arg(psessionid));
...
void WebQQNet::httpPollFinished(QNetworkReply* reply) {
  ...
  qDebug() << "httpPollFinished jsonDoc=" << jsonDoc << endl
           << "replystr=" << replystr << replystr.length() << endl;
  if (jsonDoc.isObject()) {
    ...
    if (retcode == 0) {
      ...
      for (int i = 0; i < jsonArray.size(); i++) {
        ...
        if (jsonObj.value("poll_type").toString() == QString("group_message")) {
          ...
          msg->send_uin =
              QString::number(jsonObj.value("send_uin").toDouble(), 'f', 0);
          // from_uin群TXUIN（群消息）或者好友TXUIN（私聊时）
          msg->from_uin =
              QString::number(jsonObj.value("from_uin").toDouble(), 'f', 0);
        } else if (jsonObj.value("poll_type").toString() ==
                   QString("message")) {
          jsonObj = jsonObj.value("value").toObject();
          msg->type = 0;  //好友信息
          // from_uin群TXUIN（群消息）或者好友TXUIN（私聊时）
        ...
        qDebug() << msg->type << msg->from_uin << msg->time
                 << QDateTime::fromTime_t(msg->time) << endl
                 << msg->content << endl;
      }
      WebQQ::mutex.unlock();
      emit msgReceived();
...
void WebQQNet::getGroupMemberInfo(QString groupTXUIN) {
  ...
  QString urlstr =
      QString(
          "http://s.web2.qq.com/api/"
          "get_group_info_ext2?gcode=%1&cb=undefined&vfwebqq=%2&t=%3")
          .arg(groupCode)
          .arg(vfwebqq)
...
void WebQQNet::httpExtInfoFinished(QNetworkReply* reply) {
  ...
  if (jsonDoc.isObject()) {
    ...
    qDebug() << "httpExtInfoFinished failure" << replystr
             << jsonError.errorString() << endl
             << replyData.data() << endl;
  }
  emit sysMsg(QString("获取%1群成员信息完成，成员数量：%2")
                  .arg(WebQQ::qqGroups.value(currGroupTXUIN)->name)
                  .arg(WebQQ::qqGroups.value(currGroupTXUIN)->members.size()));
  emit msgReceived();
  currGroupTXUIN.clear();
  qDebug() << "httpExtInfoFinished "
              "////////////////////////////////////////////////////////////////"
              "/////////////"
           << endl;
  // this->pollMsg();
...
void WebQQNet::httpFinished(QNetworkReply* reply) {
  ...
  switch (httpAction) {
      ...
      if (jsonDoc.isObject()) {
        ...
        if (jsonObj.value("retcode").toDouble() == 0) {
          ...
          for (int i = 0; i < jsonArray.size(); i++) {
            // jsonObj=jsonArray.at(i).toObject();
            QQfriend* f = new QQfriend();
            f->txuin = QString::number(
                jsonArray.at(i).toObject().value("uin").toDouble(), 'f', 0);
            f->markname =
                jsonArray.at(i).toObject().value("markname").toString();
            WebQQ::qqFriends.insert(f->txuin, f);
          }
          jsonArray = jsonObj.value("info").toArray();
          for (int i = 0; i < jsonArray.size(); i++) {
            // jsonObj=jsonArray.at(i).toObject();
            QString txuin = QString::number(
                jsonArray.at(i).toObject().value("uin").toDouble(), 'f', 0);
            ...
            item->setText(
                f->markname.isEmpty()
                    ? f->nick
                    : QString("%1(%2)").arg(f->nick).arg(f->markname));
    ...
    case HttpAction::SendBuddyMsgAction:
      ...
      qDebug() << "SendBuddyMsgAction" << currSendMsg << isok << replystr
               << endl;
      httpAction = HttpAction::NoAction;
      break;
    case HttpAction::SendGroupMsgAction:
      ...
      emit sendMsgFinished(currSendMsg.first, currSendMsg.second, isok);
      qDebug() << "SendGroupMsgAction" << currSendMsg << isok << replystr
               << endl;
      httpAction = HttpAction::NoAction;
      break;
...
void WebQQNet::checkVerifyCode(QString qqnum) {
  QNetworkRequest request;
  request.setUrl(QUrl(QString("http://check.ptlogin2.qq.com/"
                              "check?uin=%1&appid=1003903&js_ver=10041")
                          .arg(qqnum)));
  request.setRawHeader(
      "Accept",
...
void WebQQNet::getVerifyImg(QString qqnum) {
  QNetworkRequest request;
  request.setUrl(QUrl(
      QString("http://captcha.qq.com/getimage?aid=1003903&uin=%1").arg(qqnum)));
  request.setRawHeader(
      "Accept",
      "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8");
  // request.setRawHeader("Accept-Charset","GBK,utf-8;q=0.7,*;q=0.3");
...
void WebQQNet::login(QString qqnum, QString qqpass, QString verifyCode) {
  ...
  QString urlstr =
      QString(
          ...
          "login?u=%1&p=%2&verifycode=%3&webqq_type=10&remember_uin=1&login2qq="
          "1&aid=1003903&u1=%4&h=1&ptredirect=0&ptlang=2052&daid=164&from_ui=1&"
          "pttype=1&dumy=&fp=loginerroralert&action=2-11-12939&mibao_css=m_"
          "webqq&t=1&g=1&js_type=0&js_ver=10041&login_sig=Ip4V0YQezz6DEwpsE*-"
          "Cq*4NWVecBqWZYbNS22JCEQIT-sp20PyQtj8f-pqSIREa")
          .arg(qqnum)
          .arg(getP(qqpass, verifyCode, uinhexstr))
          .arg(verifyCode)
          .arg(
              "http%3A%2F%2Fweb2.qq.com%2Floginproxy.html%3Flogin2qq%3D1%"
              "26webqq_type%3D10");
  request.setUrl(QUrl(urlstr));
  // qDebug()<<urlstr<<endl;
  request.setRawHeader(
...
void WebQQNet::login2() {
  ...
  QByteArray postData;
  postData.append(
      "r=" + QUrl::toPercentEncoding(
                 QString("{\"status\":\"online\",\"ptwebqq\":\"%1\",\"passwd_"
                         "sig\":\"\",\"clientid\":\"%2\",\"psessionid\":null}")
                     .arg(ptwebqq)
                     .arg(clientid)));
...
void WebQQNet::loginout() {
  ...
  QString urlstr =
      QString(
          ...
          "change_status2?newstatus=offline&clientid=%1&psessionid=%2&t=%3")
          .arg(clientid)
          .arg(psessionid)
          .arg(QDateTime::currentMSecsSinceEpoch());
  request.setUrl(QUrl(urlstr));
...
void WebQQNet::getFriendInfo(QString qqnum) {
  ...
  QString urlstr = QString(
                       "http://s.web2.qq.com/api/"
                       "get_friend_info2?tuin=%1&verifysession=&code=&vfwebqq=%"
                       "2&t=1377829770859")
                       .arg(qqnum)
                       .arg(vfwebqq);
  request.setUrl(QUrl(urlstr));
  ...
  request.setRawHeader("Host", "s.web2.qq.com");
  request.setRawHeader(
      "Referer", "http://d.web2.qq.com/proxy.html?v=20110412001&callback=2");
  request.setRawHeader("User-Agent",
                       "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 "
                       "(KHTML, like Gecko) Chrome/29.0.1547.62 Safari/537.36");
...
void WebQQNet::getUserFriends() {
  ...
  postData.append(
      "r=" + QUrl::toPercentEncoding(
                 QString("{\"h\":\"hello\",\"hash\":\"%1\",\"vfwebqq\":\"%2\"}")
                     .arg(getHash(uin, ptwebqq))
                     .arg(vfwebqq)));
  request.setHeader(QNetworkRequest::ContentLengthHeader, postData.size());
...
void WebQQNet::sendBuddyMsg(QString uin, QString msg) {
  ...
  QString tmpdata =
      QString(
          "{\"to\":%1,\"face\":492,\"content\":\"[\\\"%2\\\",\\\"\\\",["
          "\\\"font\\\",{\\\"name\\\":\\\"宋体\\\",\\\"size\\\":\\\"9\\\","
          "\\\"style\\\":[0,0,0],\\\"color\\\":\\\"FF0000\\\"}]]\",\"msg_id\":%"
          "3,\"clientid\":\"%4\",\"psessionid\":\"%5\"}")
          .arg(uin)
          .arg(msg)
          .arg(msgId++)
          .arg(clientid)
...
void WebQQNet::sendGroupMsg(QString groupuin, QString msg) {
  ...
  QString tmpdata =
      QString(
          "{\"group_uin\":%1,\"content\":\"[\\\"%2\\\",\\\"\\\",[\\\"font\\\",{"
          "\\\"name\\\":\\\"宋体\\\",\\\"size\\\":\\\"9\\\",\\\"style\\\":[0,0,"
          "0],\\\"color\\\":\\\"FF0000\\\"}]]\",\"msg_id\":%3,\"clientid\":\"%"
          "4\",\"psessionid\":\"%5\"}")
          .arg(groupuin)
          .arg(msg)
          .arg(msgId++)
...
QString WebQQNet::getP(QString qqpass, QString verifyCode, QString uinhexstr) {
  QByteArray qqpassbyte =
      QCryptographicHash::hash(qqpass.toLatin1(), QCryptographicHash::Md5);
  QByteArray uinhexbyte = QByteArray::fromHex(uinhexstr.toLatin1());
  qDebug() << "uinhexbyte=" << uinhexbyte << "uinstr=" << uinhexstr << endl;
  QByteArray qqpassuinbyte =
      QCryptographicHash::hash(qqpassbyte + uinhexbyte, QCryptographicHash::Md5)
          .toHex()
          .toUpper();
  QByteArray byteP =
      QCryptographicHash::hash(qqpassuinbyte + verifyCode.toUpper().toLatin1(),
...
QString WebQQNet::getCookie(const QString& name) {
  ...
  foreach (QNetworkCookie cookie,
           http->cookieJar()->cookiesForUrl(QUrl("http://www.qq.com/"))) {
    // qDebug()<<cookie<<endl;
    if (cookie.name() == name) {
      retstr = cookie.value();
    }
    ...
    if (!(c.first >= c.second || c.first < 0 || c.second >= ja.size())) {
      if (c.first + 1 == c.second) {
        ...
        for (l = c.first, J = c.second, f = ja[c.first]; c.first < c.second;) {
          ...
          (c.first < c.second) && (ja[c.first] = ja[c.second], c.first++,
                                   r[1] = r[1] * 13 + (43 & 255));
          for (; c.first < c.second && ja[c.first] <= f;) {
            c.first++, r[2] = r[2] - (3 & 255);
          }
          c.first < c.second &&
              (ja[c.second] = ja[c.first], c.second--,
