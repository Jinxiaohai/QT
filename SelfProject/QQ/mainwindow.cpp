...
MainWindow::MainWindow(QWidget *parent)
  ...
  connect(WebQQ::webQQNet, SIGNAL(sendMsgFinished(QString, QString, bool)),
          this, SLOT(sendMsgFinished(QString, QString, bool)));
  connect(WebQQ::webQQNet, SIGNAL(sysMsg(QString)), this,
          SLOT(sysMsg(QString)));

  ui->qqGroupList->setModel(WebQQ::qqGroupListModel);
  ui->qqFriendList->setModel(WebQQ::qqFriendListModel);
  connect(ui->qqGroupList, SIGNAL(pressed(QModelIndex)), this,
          SLOT(qqGroupSelected(QModelIndex)));
  connect(ui->qqFriendList, SIGNAL(pressed(QModelIndex)), this,
          SLOT(qqFriendSelected(QModelIndex)));
  if (!loadPlugin()) {
    // QMessageBox::information(this, "Error", "Could not load the robot
    // plugin");
    ;  //没有机器人也不应该报错
  }
...
void MainWindow::qqFriendSelected(QModelIndex mi) {
  ...
  currModelIndex = mi;
  QQfriend *f =
      WebQQ::qqFriends.value(ui->txuinLineEdit->text().mid(3), nullptr);
  if (f != nullptr) {
...
bool MainWindow::loadPlugin() {
  ...
  if (!robots.isEmpty()) {
    ...
    connect(robotButtonGroup, SIGNAL(buttonClicked(int)), this,
            SLOT(robotsSelected(int)));
  }
...
void MainWindow::robotsSelected(int id) {
  ...
  if (txuinflag == "FTX") {
    ...
    if (f) {
      f->robots = tmpvector;
      WebQQ::qqFriendListModel->itemFromIndex(currModelIndex)
          ->setText(
              QString("%1%2")
                  .arg(f->markname.isEmpty()
                           ? f->nick
                           : QString("%1(%2)").arg(f->nick).arg(f->markname))
                  .arg(f->robots.size() > 0
                           ? QString("(%1个机器人)").arg(f->robots.size())
                           : ""));
    }
    ...
    if (g) {
      g->robots = tmpvector;
      WebQQ::qqGroupListModel->itemFromIndex(currModelIndex)
          ->setText(QString("%1%2").arg(g->name).arg(
              g->robots.size() > 0
                  ? QString("(%1个机器人)").arg(g->robots.size())
                  : ""));
    }
  }
}
      ...
      if (f == nullptr) {
        ...
        for (int row = 0; row < WebQQ::qqFriendListModel->rowCount(); ++row) {
          ...
          if (item->data(Qt::UserRole) == selectedtxt) {
            ...
            qDebug() << "getTXUIN currModelIndex1" << currModelIndex.data()
                     << endl;
            currModelIndex = WebQQ::qqFriendListModel->indexFromItem(item);
            qDebug() << "getTXUIN currModelIndex2" << currModelIndex.data()
                     << endl;
            ui->qqFriendList->pressed(currModelIndex);
            ui->qqFriendList->selectionModel()->select(
                currModelIndex, QItemSelectionModel::Select);
            break;
          }
        }
      }
      ...
      if (g != nullptr) {
        ...
        for (int row = 0; row < WebQQ::qqGroupListModel->rowCount(); ++row) {
          ...
          if (item->data(Qt::UserRole) == selectedtxt) {
            ...
            ui->qqGroupList->selectionModel()->select(
                currModelIndex, QItemSelectionModel::Select);
            break;
          }
        }
...
void MainWindow::msgReceived() {
  ...
  for (iMsg = WebQQ::qqmsgs.begin(); iMsg != WebQQ::qqmsgs.end();) {
    pMsg = *iMsg;
    if (pMsg->type == 1) {
      if (WebQQ::qqGroups.value(pMsg->from_uin)->members.isEmpty() &&
          WebQQ::status == QQstatus::online) {
        qDebug() << WebQQ::qqGroups.value(pMsg->from_uin)->name
                 << "下载群成员信息....." << endl;
...
QString MainWindow::formatMsg(QQmsg *msg) {
  ...
  QQfriend *f = nullptr;
  const char *fchar =
      "<font color='dimgray'>%1</font> <font color='darkolivegreen'>%2</font> "
      "<font color='black'>%3</font><br> <b>%4</b><br>";
  switch (msg->type) {
    case 0:  //好友信息
      f = WebQQ::qqFriends.value(msg->from_uin, nullptr);
      retstr =
          QString(fchar)
              .arg(QDateTime::fromTime_t(msg->time).toString(
                  "yyyy-MM-dd hh:mm:ss"))
...
void MainWindow::sysMsg(QString msg) {
  ui->textBrowser->append(
      QString("%1 <font color='red'>%2</font><br>")
...
void MainWindow::sendMsgFinished(QString txuin, QString msg, bool isok) {
  ...
  const char *fchar =
      "<font color='dimgray'>%1</font> 你对 %2 %3 说：<br>%4<br><font "
      "color='red'>%5</font>";
  QString saystr;
  txuin = txuin.mid(3);
  qDebug() << "sendMsgFinished" << txuinflag << "txuin=" << txuin << msg << isok
           << endl;
  if (txuinflag == "FTX") {
    QQfriend *f = WebQQ::qqFriends.value(txuin, nullptr);
    saystr =
...
void MainWindow::sendMsg() {
  ...
  if (txuinflag == "FTX" || txuinflag == "GTX") {
    WebQQ::webQQNet->sendMsg(ui->txuinLineEdit->text(),
