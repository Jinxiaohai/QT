...
// Fri Aug 24 10:04:58 2018 by ROOT version 6.13/02
...
class AMPT {
 public:
  ...
  Int_t Event_NELT;
  Int_t Event_NINT;
  Int_t ID[MAXMULTI];      //[multi]
  Float_t Px[MAXMULTI];    //[multi]
  Float_t Py[MAXMULTI];    //[multi]
  Float_t Pz[MAXMULTI];    //[multi]
  Float_t Mass[MAXMULTI];  //[multi]
  Float_t X[MAXMULTI];     //[multi]
  Float_t Y[MAXMULTI];     //[multi]
  Float_t Z[MAXMULTI];     //[multi]
  Float_t Time[MAXMULTI];  //[multi]
...
AMPT::AMPT(TTree *tree) : fChain(0) {
  ...
  if (tree == 0) {
    TFile *f =
        (TFile *)gROOT->GetListOfFiles()->FindObject("ampt_after_art.root");
    if (!f || !f->IsOpen()) {
